...
def setup(chip):
    ...
    chip.set('tool', tool, 'version', '>=v2.0-13145', clobber=False)
...
def define_floorplan_params(chip):
...
def define_ppl_params(chip):
    ...
    pdkname = chip.get('option', 'pdk')
    stackup = chip.get('option', 'stackup')
    chip.add('tool', tool, 'task', task, 'require', ",".join(['pdk', pdkname, 'var', tool, 'pin_layer_horizontal', stackup]), step=step, index=index)
    chip.add('tool', tool, 'task', task, 'require', ",".join(['pdk', pdkname, 'var', tool, 'pin_layer_vertical', stackup]), step=step, index=index)
